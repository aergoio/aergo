...
func newBlockExecutor(bc *DummyChain, txs []*types.Tx) (*blockExecutor, error) {
	...
	scs, err := state.GetSystemAccountState(blockState.LuaStateDB)
	if err != nil {
		return nil, err
	}
	...
	return &blockExecutor{
		...
		//	return cs.validator.ValidatePost(blockState.GetLuaRoot(), blockState.Receipts(), block)
		//},
		bi: bi,
	}, nil
...
func executeGovernanceTx(ccc consensus.ChainConsensusCluster, bs *state.BlockState, txBody *types.TxBody, sender, receiver *state.AccountState,
	...
	scs, err := state.OpenContractState(receiver.AccountID(), receiver.State(), bs.LuaStateDB)
	if err != nil {
		return nil, err
	}
	...
	if err == nil {
		err = state.StageContractState(scs, bs.LuaStateDB)
	}
