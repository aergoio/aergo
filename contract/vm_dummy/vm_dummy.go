...
func contractFrame(l luaTxContract, bs *state.BlockState, cdb contract.ChainAccessor, receiptTx db.Transaction,
	...
	creatorState, err := state.GetAccountState(l.sender(), bs.LuaStateDB, bs.EthStateDB)
	if err != nil {
		return err
	}

	contractId := types.ToAccountID(l.recipient())
	contractState, err := state.GetAccountState(l.recipient(), bs.LuaStateDB, bs.EthStateDB)
	if err != nil {
		return err
	}
