    ...
    for i, cmd in ipairs(commands) do
        ...
        elseif action == "pcall" then
            ...
            if to_call == name:get() then
                xpcall(test, error_handler, script)
            elseif amount ~= nil then
                ...
                local address = resolve(to_call)
                success, ret = xpcall(contract.call, error_handler, address, "test", script)
            end
        ...
        elseif action == "deploy" then
            ...
            xpcall(contract.deploy, error_handler, code_or_address, unpack(cmd,3))
