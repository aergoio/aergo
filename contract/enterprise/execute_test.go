...
func TestBasicFailEnterprise(t *testing.T) {
	...
	_, err := ExecuteEnterpriseTx(nil, ccc, scs, tx, sender, testBlockNo)
...
func TestBasicEnterprise(t *testing.T) {
	...
	tx.Payload = []byte(`{"name":"appendAdmin", "args":["AmPNYHyzyh9zweLwDyuoiUuTVCdrdksxkRWDjVJS76WQLExa2Jr4"]}`)
	event, err := ExecuteEnterpriseTx(nil, ccc, scs, tx, sender, testBlockNo)
	...
	_, err = ExecuteEnterpriseTx(bs, ccc, scs, tx, sender, testBlockNo)
