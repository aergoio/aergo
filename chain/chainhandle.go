...
func (ec *ErrBlock) Error() string {
	return fmt.Sprintf("Error: %s. block(%s, %d)", ec.err.Error(), enc.B58Encode(ec.block.Hash), ec.block.No)
}
...
func (ec *ErrTx) Error() string {
	return fmt.Sprintf("error executing tx:%s, tx=%s", ec.err.Error(), enc.B58Encode(ec.tx.GetHash()))
}
...
func (cp *chainProcessor) addBlock(blk *types.Block) error {
	...
	if logger.IsDebugEnabled() {
		logger.Debug().Bool("isMainChain", cp.isMainChain).
			...
			Str("hash", blk.ID()).
			Str("prev_hash", enc.B58Encode(blk.GetHeader().GetPrevBlockHash())).
...
func NewTxExecutor(execCtx context.Context, ccc consensus.ChainConsensusCluster, cdb contract.ChainAccessor, bi *types.BlockHeaderInfo, preloadService int) TxExecFn {
	return func(bState *state.BlockState, tx types.Transaction) error {
		...
		if err != nil {
			logger.Error().Err(err).Str("hash", enc.B58Encode(tx.GetHash())).Msg("tx failed")
...
func executeTx(execCtx context.Context, ccc consensus.ChainConsensusCluster, cdb contract.ChainAccessor, bs *state.BlockState, tx types.Transaction, bi *types.BlockHeaderInfo, preloadService int) error {
	...
	case types.TxType_GOVERNANCE:
		...
		if err != nil {
			logger.Warn().Err(err).Str("txhash", enc.B58Encode(tx.GetHash())).Msg("governance tx Error")
		}
	case types.TxType_FEEDELEGATION:
		...
		if err != nil {
			if err != types.ErrNotAllowedFeeDelegation {
				logger.Warn().Err(err).Str("txhash", enc.B58Encode(tx.GetHash())).Msg("checkFeeDelegation Error")
				return err
			}
