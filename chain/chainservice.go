...
func (core *Core) initGenesis(genesis *types.Genesis, mainnet bool, testmode bool) (*types.Block, error) {
	...
	logger.Info().Str("chain id", gen.ID.ToJSON()).
		Str("hash", enc.B58Encode(genesisBlock.GetHash())).Msg("chain initialized")
	...
	if ConsensusName() == consensus.ConsensusName[consensus.ConsensusDPOS] {
		...
		if err != nil {
			...
			for _, res := range top.Votes {
				logger.Debug().Str("BP", enc.B58Encode(res.Candidate)).
		...
		if msg.Bstate != nil {
			...
			if timeoutTx := bstate.TimeoutTx(); timeoutTx != nil {
				if logger.IsDebugEnabled() {
					logger.Debug().Str("hash", enc.B58Encode(timeoutTx.GetHash())).Msg("received timeout tx")
				}
...
func getAddressNameResolved(sdb *state.StateDB, account []byte) ([]byte, error) {
	if len(account) == types.NameLength {
		...
		if err != nil {
			logger.Error().Str("hash", enc.B58Encode(account)).Err(err).Msg("failed to get state for account")
			return nil, err
		}
...
func (cw *ChainWorker) Receive(context actor.Context) {
	...
	getAccProof := func(sdb *state.StateDB, account, root []byte, compressed bool) (*types.AccountProof, error) {
		...
		if err != nil {
			logger.Error().Str("hash", enc.B58Encode(address)).Err(err).Msg("failed to get state for account")
			return nil, err
		}
		proof.Key = address
		return proof, err
	...
	case *message.GetBlock:
		...
		if err != nil {
			logger.Debug().Err(err).Str("hash", enc.B58Encode(msg.BlockHash)).Msg("block not found")
		}
	...
	case *message.GetQuery:
		...
		if err != nil {
			logger.Error().Str("hash", enc.B58Encode(address)).Err(err).Msg("failed to get state for contract")
			context.Respond(message.GetQueryRsp{Result: nil, Err: err})
		} else {
	...
	case *message.GetStateQuery:
		...
		if contractProof.Inclusion {
			...
			for _, storageKey := range msg.StorageKeys {
				...
				if err != nil {
					logger.Error().Str("hash", enc.B58Encode(contractProof.Key)).Err(err).Msg("failed to get state variable in contract")
				}
			}
		}
	...
	case *message.CheckFeeDelegation:
		...
		if err != nil {
			logger.Error().Str("hash", enc.B58Encode(msg.Contract)).Err(err).Msg("failed to get state for contract")
