start_nodes() {
  ...
  if [ "$consensus" == "sbp" ]; then
    ...
    # three local nodes only (no Docker peers)
...
wait_version_from() {
  ...
  while true; do
    ...
    if [[ -n "$output" ]] && [[ "${output:0:1}" == "{" ]]; then
      # Support both legacy and current aergocli JSON field casing.
      cur_version=$(echo "$output" | jq -r '(.chainInfo.id.version // .ChainInfo.Chainid.Version) // empty')
      if [ -z "$cur_version" ] || [ "$cur_version" == "null" ]; then
        cur_version=0
      fi
    else
      cur_version=0
    ...
    if [ $cur_version -eq $expect_version ]; then
      ...
      sleep 0.5
      counter=$((counter+1))
      # ~30s: nodes can be slow to reload config and report the new hardfork version.
...
get_receipt_from() {
  ...
  # wait up to ~100s on "tx not found" (slow follower / raft apply lag)
...
get_receipt() {
  ...
  for retry in $(seq 1 $max_retries); do
    # get receipts from all running nodes
    ...
    # compare receipts - they must be the same (canonical JSON; field order may differ)
    all_match=true
    for i in 2 3; do
      local diff_output=$(diff <(jq -S . receipt1.json) <(jq -S . receipt${i}.json))
  ...
  rm receipt2.json receipt3.json
}
