...
if [ "$consensus" == "sbp" ]; then
  ...
  # delete and recreate node dirs (three local nodes only)
  rm -rf node1
  rm -rf node2
  rm -rf node3
  mkdir node1
...
function set_version() {
  ...
  block_no=$(../bin/aergocli blockchain | jq -r '.height // .Height // empty' | sed 's/"//g')
  # increment 3 numbers
  block_no=$((block_no+3))
...
# Determine the highest hardfork version the compiled aergosvr knows about.
# config/hardfork.json (in the source tree) lists every version the binary was
# generated for. If the binary on this branch only supports up to v2, asking
# the chain to advance to v3+ is a silent no-op (toml `v3=...` is unknown to
# the parser) and `wait_version` will hang forever.
max_fork_version=2
if [ -f ../config/hardfork.json ]; then
  hjson_max=$(jq -r '[.[].Version] | max' ../config/hardfork.json 2>/dev/null)
  if [ -n "$hjson_max" ] && [ "$hjson_max" != "null" ]; then
    max_fork_version=$hjson_max
  fi
fi
echo "binary supports hardfork versions up to v$max_fork_version"
