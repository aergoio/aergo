...
initial_block_height=$(../bin/aergocli blockchain | jq -r '.height // .Height // empty')
...
if [ $query_exit_code -ne 0 ]; then
  ...
  if [ -z "$query_result" ]; then
    ...
    cleaned_result=$(echo "$query_result" | sed 's/"//g' | sed 's/\\//g' | sed 's/value://g' | xargs)
...
if [ $state_exit_code -ne 0 ]; then
  ...
  if [ -z "$state_result" ]; then
    ...
    # aergocli prints the full StateQueryProof (protobuf text); only require the stored value to appear.
    assert_contains "$state_result" "testname"
    echo "✓"
  fi
fi
...
if [ $state_exit_code -ne 0 ]; then
  ...
  if [ -z "$state_result" ]; then
    ...
    assert_contains "$state_result" "testarray"
    echo "✓"
  fi
fi
...
if [ $state_exit_code -ne 0 ]; then
  ...
  if [ -z "$state_result" ]; then
    ...
    assert_contains "$state_result" "testvalue"
    echo "✓"
  fi
fi
