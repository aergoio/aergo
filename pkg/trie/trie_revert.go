...
func (s *Trie) maybeDeleteSubTree(original []byte, maybeDelete []byte, height uint64, batch, batch2 [][]byte, iBatch uint8, ch chan<- (error)) {
	if height == 0 {
...
func (s *Trie) deleteSubTree(root []byte, height uint64, batch [][]byte, iBatch uint8, ch chan<- (error)) {
	if len(root) == 0 {
		ch <- nil
		return
	}
