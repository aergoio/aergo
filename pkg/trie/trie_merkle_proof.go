...
// on the path of the non-included key, 3- (nil, nil) for a non-included key
// with a DefaultLeaf on the path
func (s *Trie) MerkleProofPast(key, root []byte) ([][]byte, bool, []byte, []byte, error) {
	s.lock.RLock()
	defer s.lock.RUnlock()
	s.atomicUpdate = false // so loadChildren doesnt return a copy
...
func (s *Trie) MerkleProofCompressedPast(key, root []byte) ([]byte, [][]byte, int, bool, []byte, []byte, error) {
