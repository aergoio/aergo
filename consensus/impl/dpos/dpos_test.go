...
func TestDposFutureBlock(t *testing.T) {
	...
	block := types.NewBlock(nil, nil, nil, nil, nil, nil, time.Now().Add(3*time.Second).UnixNano())
	assert.True(t, !dpos.VerifyTimestamp(block), "future block check failed")

	block = types.NewBlock(nil, nil, nil, nil, nil, nil, time.Now().UnixNano())
	assert.True(t, dpos.VerifyTimestamp(block), "future block check failed")

	block = types.NewBlock(nil, nil, nil, nil, nil, nil, time.Now().Add(-time.Second).UnixNano())
	assert.True(t, dpos.VerifyTimestamp(block), "future block check failed")
...
func TestDposPastBlock(t *testing.T) {
	...
	block0 := types.NewBlock(nil, nil, nil, nil, nil, nil, time.Now().UnixNano())
	assert.True(t, dpos.VerifyTimestamp(block0), "invalid timestamp")

	time.Sleep(time.Second)
	now := time.Now().UnixNano()
	block1 := types.NewBlock(block0, bv, nil, nil, nil, nil, now)
	...
	block2 := types.NewBlock(block0, bv, nil, nil, nil, nil, now)
