...
func NewGetTxsReceiver(actor p2pcommon.ActorService, peer p2pcommon.RemotePeer, sm p2pcommon.SyncManager, txIDs []types.TxID, ttl time.Duration) *GetTxsReceiver {
	timeout := time.Now().Add(ttl)
	hashes := make([][]byte, len(txIDs))
	for i, _ := range txIDs {
		hashes[i] = txIDs[i][:]
	}
	return &GetTxsReceiver{actor: actor, peer: peer, sm: sm, hashes: hashes, timeout: timeout,}
}
