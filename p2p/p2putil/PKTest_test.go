...
func PrintLibP2PKey(priv crypto.Key, marshaled []byte, t *testing.T) {
	...
	if err != nil {
		...
		t.Logf("BT/MAR %v", enc.HexEncode(oldBytes))
		t.Logf("RAW    %v", enc.HexEncode(newBytes))
	}
}

func PrintBTCPKey(priv *btcec.PrivateKey, t *testing.T) {
	oldBytes := priv.Serialize()
	t.Logf("PRIV   %v", enc.HexEncode(oldBytes))
	t.Logf("PUBLIC %v", enc.HexEncode(priv.PubKey().SerializeCompressed()))
}
