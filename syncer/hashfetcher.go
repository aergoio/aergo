...
func (hf *HashFetcher) requestHashSet() {
	...
	logger.Debug().Uint64("prev", hf.lastBlockInfo.No).Str("prevhash", enc.B58Encode(hf.lastBlockInfo.Hash)).Uint64("count", count).Msg("request hashset to peer")
...
func (hf *HashFetcher) isValidResponse(msg *message.GetHashesRsp) (bool, error) {
	...
	if !isValid {
		logger.Error().Str("req prev", enc.B58Encode(hf.lastBlockInfo.Hash)).
			Str("msg prev", enc.B58Encode(msg.PrevInfo.Hash)).
...
func (hf *HashFetcher) GetHahsesRsp(msg *message.GetHashesRsp) {
	...
	logger.Debug().Int("count", count).
		...
		Str("start", enc.B58Encode(msg.Hashes[0])).
		Str("end", enc.B58Encode(msg.Hashes[count-1])).Msg("receive GetHashesRsp")
