...
func (bf *BlockFetcher) checkTaskTimeout() error {
	...
	for e := bf.runningQueue.Front(); e != nil; e = next {
		...
		logger.Error().Uint64("StartNo", task.startNo).Str("start", enc.B58Encode(task.hashes[0])).Int("cout", task.count).Int("runqueue", bf.runningQueue.Len()).Int("pendingqueue", bf.pendingQueue.Len()).
...
func (bf *BlockFetcher) processFailedTask(task *FetchTask, isErr bool) error {
	...
	logger.Error().Int("peerno", task.syncPeer.No).Uint64("StartNo", task.startNo).Str("start", enc.B58Encode(task.hashes[0])).Msg("task fail, move to retry queue")
...
func (bf *BlockFetcher) popNextTask(task *FetchTask) {
	logger.Debug().Int("retry", task.retry).Uint64("StartNo", task.startNo).Str("start", enc.B58Encode(task.hashes[0])).Str("end", enc.B58Encode(task.hashes[task.count-1])).
...
func (bf *BlockFetcher) searchCandidateTask() (*FetchTask, error) {
	...
	addNewFetchTasks := func(hashSet *HashSet) {
		...
		logger.Debug().Uint64("startno", hashSet.StartNo).Str("start", enc.B58Encode(hashSet.Hashes[0])).Int("count", hashSet.Count).Msg("add new fetchtasks from HashSet")
	...
	if bf.retryQueue.Len() > 0 {
		...
		if bf.pendingQueue.Len() == 0 {
			...
			logger.Debug().Uint64("startno", hashSet.StartNo).Array("hashes", &LogBlockHashesMarshaller{hashSet.Hashes, 10}).Str("start", enc.B58Encode(hashSet.Hashes[0])).Int("count", hashSet.Count).Msg("BlockFetcher got hashset")
...
func (bf *BlockFetcher) runTask(task *FetchTask, peer *SyncPeer) {
	...
	logger.Debug().Int("peerno", task.syncPeer.No).Int("count", task.count).Uint64("StartNo", task.startNo).Str("start", enc.B58Encode(task.hashes[0])).Int("runqueue", bf.runningQueue.Len()).Msg("send block fetch request")
	...
	for e := bf.runningQueue.Front(); e != nil; e = next {
		...
		if peerMatch {
			if task.isPeerMatched(msg.ToWhom) {
				...
				logger.Debug().Stringer("peer", types.LogPeerShort(msg.ToWhom)).Err(msg.Err).Str("start", enc.B58Encode(task.hashes[0])).Int("count", task.count).Int("runqueue", bf.runningQueue.Len()).Msg("task finished with error")
				return task, nil
			}
		} else {
			//find finished peer
			if task.isMatched(msg.ToWhom, msg.Blocks, count) {
				...
				logger.Debug().Uint64("StartNo", task.startNo).Str("start", enc.B58Encode(task.hashes[0])).Int("count", task.count).Int("runqueue", bf.runningQueue.Len()).
					Msg("task finished")
...
func (task *FetchTask) isTimeOut(now time.Time, timeout time.Duration) bool {
	if now.Sub(task.started) > timeout {
		logger.Info().Int("peerno", task.syncPeer.No).Uint64("startno", task.startNo).Str("start", enc.B58Encode(task.hashes[0])).Int("cout", task.count).Msg("FetchTask peer timeouted")
		return true
	}
...
func (task *FetchTask) isMatched(peerID types.PeerID, blocks []*types.Block, count int) bool {
	...
	for i, block := range blocks {
		if bytes.Compare(task.hashes[i], block.GetHash()) != 0 {
			logger.Info().Int("peerno", task.syncPeer.No).Str("hash", enc.B58Encode(task.hashes[0])).Int("idx", i).Msg("task hash mismatch")
			return false
		}
	}
