...
func TestBasicExecute(t *testing.T) {
	...
	_, err := ExecuteSystemTx(testProposals, testVotingCatalog, scs, emptytx, sender, receiver, blockInfo)
	assert.EqualError(t, types.ErrTxInvalidPayload, err.Error(), "Execute system tx failed")

	events, err := ExecuteSystemTx(testProposals, testVotingCatalog, scs, tx.GetBody(), sender, receiver, blockInfo)
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, tx.GetBody(), sender, receiver, blockInfo)
...
func TestValidateSystemTxForStaking(t *testing.T) {
	...
	sender.AddBalance(types.StakingMinimum)
	blockInfo := &types.BlockHeaderInfo{No: uint64(0)}
	_, err = ValidateSystemTx(testProposals, testVotingCatalog, tx.Body.Account, tx.GetBody(), sender, scs, blockInfo)
...
func TestValidateSystemTxForUnstaking(t *testing.T) {
	...
	blockInfo := &types.BlockHeaderInfo{No: uint64(0)}
	_, err = ValidateSystemTx(testProposals, testVotingCatalog, tx.Body.Account, tx.GetBody(), nil, scs, blockInfo)
...
func TestValidateSystemTxForVoting(t *testing.T) {
	...
	blockInfo.No += StakingDelay
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, unStakingTx.GetBody(), sender, receiver, blockInfo)
...
func TestProposalExecute(t *testing.T) {
	...
	event, err := ExecuteSystemTx(testProposals, testVotingCatalog, scs, votingTx.GetBody(), sender, receiver, blockInfo)
	...
	vinfo, err := GetVotes(scs, testVotingCatalog, sender.ID())
...
func TestProposalExecuteFail1(t *testing.T) {
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, invalidaVersionTx.GetBody(), sender, receiver, blockInfo)
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, tooEarlyTx.GetBody(), sender, receiver, blockInfo)
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, tooManyCandiTx.GetBody(), sender, receiver, blockInfo)
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, tooLateTx.GetBody(), sender, receiver, blockInfo)
...
func TestProposalExecute2(t *testing.T) {
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, votingTx.GetBody(), sender, receiver, blockInfo)
	assert.NoError(t, err, "failed in voting proposal")
	votingTx.Body.Account = sender2.ID()
	votingTx.Body.Payload = []byte(`{"Name":"v1voteDAO", "Args":["BPCOUNT", "13"]}`)
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, votingTx.GetBody(), sender2, receiver, blockInfo)
	assert.NoError(t, err, "could not execute system tx")
	votingTx.Body.Account = sender3.ID()
	votingTx.Body.Payload = []byte(`{"Name":"v1voteDAO", "Args":["BPCOUNT", "13"]}`)
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, votingTx.GetBody(), sender3, receiver, blockInfo)
	assert.NoError(t, err, "could not execute system tx")
	...
	_, err = ExecuteSystemTx(testProposals, testVotingCatalog, scs, votingTx.GetBody(), sender3, receiver, blockInfo)
	assert.NoError(t, err, "could not execute system tx")
	gasPrice := TestParams.GetGasPrice()
	...
	currentNamePrice := TestParams.GetNamePrice()
	assert.Equal(t, "1004", currentNamePrice.String(), "current name price")

	/*
		blockInfo += StakingDelay
		//voting result was freeze
		_, err = ExecuteSystemTx(TestProposals, scs, unstakingTx.GetBody(), sender, receiver, blockInfo)
		assert.NoError(t, err, "could not execute system tx")
		assert.Equal(t, new(big.Int).Sub(balance3, types.ProposalPrice), sender.Balance(), "sender.Balance() should be 2 after unstaking")
